...
# 配置路径
image_path = r"E:\pyLearn\imgs\cm1.png"
...
def detect_text_boxes(image_path):
    ...
    for cnt in contours:
        ...
        if (0.2 < aspect_ratio < 10) and (w * h > 500):
...
def translate_with_deepseek(text, source_lang='ja', target_lang='zh'):
...
def main():
    # 1. 检测文本框
    image, text_boxes = detect_text_boxes(image_path)
    
    # 2. 提取文本
    extracted_texts = extract_text_from_boxes(image, text_boxes)
    ...
    for item in extracted_texts:
        ...
        translated_text = translate_with_deepseek(original_text)
